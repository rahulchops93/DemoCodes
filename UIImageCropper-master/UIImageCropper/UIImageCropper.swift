...
public class UIImageCropper: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    override public func viewDidLoad() {
        ...
        let ratioConst = NSLayoutConstraint(item: cropView, attribute: .width, relatedBy: .equal, toItem: cropView, attribute: .height, multiplier: 0.25/*cropRatio*/, constant: 0)
        let heightCropConst = NSLayoutConstraint(item: cropView, attribute: .height, relatedBy: .equal, toItem: cropView, attribute: .height, multiplier: 0.25/*cropRatio*/, constant: 0)
//        cropView.addConstraints([ratioConst, heightCropConst])
        
        NSLayoutConstraint.activate([
            cropView.centerXAnchor.constraint(equalTo: topView.centerXAnchor),
            cropView.centerYAnchor.constraint(equalTo: topView.centerYAnchor),
            cropView.widthAnchor.constraint(equalToConstant: 100),
            cropView.heightAnchor.constraint(equalToConstant: 100),
            
            topView.widthAnchor.constraint(equalTo: self.view.widthAnchor),
            topView.heightAnchor.constraint(equalTo: self.view.heightAnchor)
        ])
...
//        cropView.backgroundColor = UIColor.red
    ...
    override public func viewWillAppear(_ animated: Bool) {
        ...
        cropView.layer.cornerRadius = cropView.frame.size.height / 2
//        fadeView.layer.cornerRadius = fadeView.frame.size.height / 2
        self.maskFadeView()
    ...
    override public func viewDidLayoutSubviews() {
        ...
        imageWidthConst?.constant = self.view.frame.size.width
        imageHeightConst?.constant = self.view.frame.size.height
...
//        let path = UIBezierPath(rect: cropView.frame)
        let path = UIBezierPath(roundedRect: cropView.frame, cornerRadius: cropView.frame.size.height / 2)
...
//        let cropFrame = CGRect(x: x * imageSize.width, y: y * imageSize.height, width: imageSize.width * width, height: imageSize.height * height)
        let cropFrame = CGRect(x: x * imageSize.width, y: y * imageSize.height, width: imageSize.width * 100, height: imageSize.height * 100)
